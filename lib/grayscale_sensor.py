...
class grayscale_Interpreter():
    ...
    def carRelativePosition2Line(self,grayscale_sensor_bus_value):
        ...
        grayScaleValues = grayscale_sensor_bus_value
...
class grayscale_Controller():
    ...
    def calculateSteeringAngle(self, interpreter_message_bus_value):

        #while True:
        #relative position ranges from [-1,1]
        relativePosition = interpreter_message_bus_value
        #time.sleep(sleepTime)
        angle = np.interp(relativePosition, [-1,1],[-self.scalingFactor,self.scalingFactor])
        #controller_message_bus.write(angle)   
        self.Picar_motors.set_dir_servo_angle(angle)
